                                    ...
                                    <?php while ($year = mysqli_fetch_array($years)) {
                                        if ($_GET['year'] == $year['year'] || $current_year == $year['year']) { ?>
                                            <option selected value="<?php echo $_GET['year'] ?? $current_year ?>">
                                                <?php echo $_GET['year'] ?? $current_year ?>
                                            </option>
                ...
                <div class="card card-body table-responsive">
                    <table class="table table-striped" id="incomeStatment">
                        ...
                        <tbody>
                            ...
                            $expenseName = "SELECT * FROM common_expenses_types ORDER BY id";
                            $names = mysqli_query($conn, $expenseName);
                            ...
                            while ($name = mysqli_fetch_array($names)) {
                                ...
                                if ($e_name  == 'Tol Plaza') {
                                    $e_name = 'Tol';
                                } else if ($e_name == 'Hawa + Jaali') {
                                    $e_name = 'Hawa';
                                } else if ($e_name == 'Vehicle Repair') {
                                    $e_name = 'Vehicle';
                                } 
                                ...
                                if (@$_GET['year'] || @$_GET['month']) {
                                    ...
                                    SELECT e.expense_name, SUM(e.balance) AS amount
                                    FROM vehicle_expenses v
                                    JOIN vehicle_expenses_details e ON v.id = e.expense_id
                                    WHERE v.vehicle_id = '$current_vehicle_id'
                                    AND expense_name= '$e_name'
                                ...
                                ");
                                    ...
                                    $month = date('m');
                                    $expenses = mysqli_query($conn, "
                                    SELECT d.expense_name, SUM(d.balance) AS amount
                                    FROM vehicle_expenses v
                                    JOIN vehicle_expenses_details d ON v.id = d.expense_id
                                    WHERE v.vehicle_id = '$current_vehicle_id'
                                    AND MONTH(v.end_date) = $month
                                    AND d.expense_name= '$e_name'
                                    AND YEAR(v.end_date)  = $year
                                ");

                                }

                                $expense = mysqli_fetch_array($expenses);
                                $e_amount = $expense['amount'];

                                ?>
                                <tr class="bg-danger text-white">
                                    <td></td>
                                    <td class="text-white"> <?= $d_name ?></td>
                                    <td></td>
                                    <td></td>
                                    <td></td>
                                    <td></td>
                                    <td class="text-white"></td>
                                    <td class="text-white"><?php echo $e_amount ?></td>
                                </tr>
