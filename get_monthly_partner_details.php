...
if(isset($_POST['year'])){
  ...
  if($isPartner){
    ...
    WHERE status=1 AND YEAR(date) = '$year' AND p_id IN (
      SELECT id FROM partners WHERE name='$p_id'
  ) ORDER BY date ASC,id ASC";

  }else{ 
          
    $query = "SELECT * FROM partner_details WHERE p_id='$p_id' AND status=1 AND YEAR(date) = '$year' ORDER BY date ASC,id ASC";
            
  }
        $result = mysqli_query($connect, $query);

        if($isPartner){
          $preAmount = "SELECT SUM(credit) AS cash_in_value, SUM(debit) AS cash_out_value FROM partner_details WHERE status=1 AND  YEAR(date) < '$year' AND p_id IN (
            SELECT id FROM partners WHERE name='$p_id'
        )"; 
        }else{
          $preAmount = "SELECT SUM(credit) AS cash_in_value, SUM(debit) AS cash_out_value FROM partner_details WHERE p_id='$p_id' AND status=1 AND  YEAR(date) < '$year'"; 
        }
        ...
        //echo $total_cashin. " / ".$total_cashout; exit();
        if($total_cashin == ''){
            $total_cashin =0.0;
        }
          ...
          while ($row = mysqli_fetch_array($result)) {
            ...
            $res = $dat . ' ' . $month_name . ' , ' . $year;
              ...
              <td width="10%">' . $res . '</td>';
                ...
                if($type == "monthly profit"){
                  $table_result .= '<a href="partner_old_amount.php?order_id='.$id .'">' . $note . ' ('.$partnerProfitRow["month"].')</a>';
                }else{
