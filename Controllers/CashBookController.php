...
if (isset($_POST['customer_transaction']) && $_POST['customer_transaction'] == "customer_transaction") {
    ...
    if ($vehicle_id == "Company") {
        ...
        $queryFired = mysqli_query($conn, "INSERT INTO cashbook (consume_id, operation_for,vehicle_id,message ,cash_in, cash_out, date, notes, status) VALUES ('$customer_id','$operation_for','$current_vehicle_id','$message','$cash_in','$cash_out','$date','$notes','$status')");
        
        $cashbook_id = mysqli_insert_id($conn);

        $customerDetailsqueryFired = mysqli_query($conn, "INSERT INTO customer_details (customer_id, status, transaction_id, note,type, credit, debit,due,date) VALUES ('$customer_id','$status','$cashbook_id','$message','Cash Payment','$cash_in','0','0','$date')");
        ...
        $vehicleDetailsqueryFired = mysqli_query($conn, "INSERT INTO vehicle_details (vehicle_id, status, transaction_id, note,type, credit, debit,due,date) VALUES ('$current_vehicle_id','$status','$customer_id','$message','Cash Received','$cash_in','0','0','$date')");
...
if (isset($_POST['give_to_vehicle_transaction']) && $_POST['give_to_vehicle_transaction'] == "give_to_vehicle_transaction") {
    ...
    $status = 1;
    $message = mysqli_real_escape_string($conn, $_POST['note']);
    ...
    $queryFired = mysqli_query($conn, "INSERT INTO cashbook (consume_id, operation_for,vehicle_id, message ,cash_in, cash_out, date, notes, status) VALUES ('$vehicle_id','$operation_for','$vehicle_id','$message','$cash_in','$cash_out','$date','$notes','$status')");
...
if (isset($_POST['pay_to_pump_transaction']) && $_POST['pay_to_pump_transaction'] == "pay_to_pump_transaction") {
    ...
    $queryFired = mysqli_query($conn, "INSERT INTO cashbook (consume_id, operation_for,vehicle_id,message ,cash_in, cash_out, date, notes, status) VALUES ('$pump_id','$operation_for','$current_vehicle_id','$message','$cash_in','$cash_out','$date','$notes','$status')");
