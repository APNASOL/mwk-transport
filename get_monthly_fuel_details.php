...
if (isset($_POST['month'])) {
    ...
    if ($c_id != "empty") {
        ...
        // $vehicle_details = mysqli_query($conn, "SELECT date, ANY_VALUE(id) AS id, ANY_VALUE(vehicle_id) AS vehicle_id, ANY_VALUE(status) AS status FROM vehicle_details WHERE vehicle_id =  $c_id and YEAR(date) = '$year' AND MONTH(date) = '$month' GROUP BY date");
    ...
    if ($month != "" && $year != "") {
        ...
        while ($fuel = mysqli_fetch_array($fuel_array)) {
            ...
            // If $pump_id is not numeric, then it's already a name
            if (!is_numeric($pump_id)) {
                $pump_name = $pump_id; // Directly use the text value
            } else {
                // Otherwise, fetch the pump name from the database
                $pump = mysqli_query($conn, "SELECT name FROM pumps WHERE id = " . intval($pump_id));
                if ($pump && mysqli_num_rows($pump) > 0) {
                    $pump_array = mysqli_fetch_assoc($pump);
                    $pump_name = $pump_array['name'];
                } else {
                    $pump_name = "Unknown Pump";
                }
            }
