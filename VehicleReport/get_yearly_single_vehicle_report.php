...
if (isset($_POST['year'])) {
    ...
    if ($year != "") {
        ...
        $query2 = "SELECT COUNT(id) AS total_order,SUM(total_bill) AS value_sum FROM trips WHERE status=1 AND vehicle_id = '$c_id' and YEAR(end_date) = '$year' ORDER BY id ASC";
        ...
        /*$preAmount = "SELECT SUM(credit) AS credit, SUM(debit) AS debit FROM vehicle_details WHERE status=1 AND c_id='$c_id' and YEAR(date) <= '$year' AND MONTH(date) < '$month'";  */

        /*if($month == 01){
        $preAmount = "SELECT SUM(credit) AS credit, SUM(debit) AS debit FROM vehicle_details WHERE status=1 AND c_id='$c_id' and YEAR(date) < '$year'";
        ...
        $preAmount = "SELECT SUM(credit) AS credit, SUM(debit) AS debit FROM vehicle_details WHERE status = 1 AND vehicle_id='$c_id' and YEAR(date) < '$year'";
        ...
        $details = "SELECT * FROM vehicle_details WHERE status=1 AND vehicle_id = '$c_id' and YEAR(date) = '$year' ORDER by date ASC";
        ...
        if (mysqli_num_rows($detailsResult) > 0) {
              ...
              <div class="card-content collapse show">
                <div class=" card-dashboard dataTables_wrapper dt-bootstrap">
                    <div class="table-responsive">
                     <table class="table table-bordered" id="YearlyTable">
                         <thead>
                         <tr>
                         <th>Date</th>
                         <th>Customer</th>
            ...
            $table_daily_result .= '<div class="table-responsive">
                       <table class="table table-borderless" id="YearlyTable">
                         <thead>
                         <tr>
                         <th>Date</th>
                         <th>Customer</th>
