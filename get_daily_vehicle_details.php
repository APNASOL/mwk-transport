...
if (isset($_POST['date'])) {
    ...
    if ($c_id != "empty") {
    $vehicle_details = mysqli_query($conn, "SELECT * FROM vehicle_details WHERE vehicle_id =  $c_id and date = '$date' group BY date");
    ...
    if ($date) {
        ...
        if ($vehicle_details != "") {
            $preAmount = mysqli_query($conn, "SELECT * FROM vehicle_details WHERE status = 1 AND vehicle_id='$c_id' AND date < '$date'");
