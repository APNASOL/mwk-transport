...
if (isset($_POST['f_date'])) {
    ...
    if ($f_date != "" && $t_date != "") {
        ...
        $query1 = "SELECT COUNT(id) AS total_order,SUM(balance) AS value_sum FROM fuel WHERE status=1 and pump_id = '$c_id' and date Between '$f_date' And '$t_date'";
        ...
        $preAmount = "SELECT SUM(credit) AS credit, SUM(debit) AS debit FROM pumps_details WHERE status = 1 AND pump_id = '$c_id' and date < '$f_date'";
        ...
        $details = "SELECT * FROM pumps_details WHERE status = 1 and pump_id = '$c_id' and date Between '$f_date' And '$t_date' ORDER by date ASC";
