...
if (isset($_POST['f_date'])) {
    ...
    if ($f_date != "" && $t_date != "") {
        ...
        $query1 = "SELECT COUNT(id) AS total_order,SUM(total_bill) AS value_sum FROM trips WHERE status=1 and customer_id = '$c_id' and end_date Between '$f_date' And '$t_date'";
        ...
        $preAmount = "SELECT SUM(credit) AS credit, SUM(debit) AS debit FROM customer_details WHERE status = 1 AND customer_id = '$c_id' and date < '$f_date'";
        ...
        $details = "SELECT * FROM customer_details WHERE status=1 and customer_id='$c_id' and date Between '$f_date' And '$t_date' ORDER by date ASC";
