...
if (isset($_POST['year'])) {
    ...
    if ($year != "") {
        ...
        $query2 = "SELECT COUNT(id) AS total_order,SUM(total_bill) AS value_sum FROM trips WHERE status=1 AND customer_id = '$c_id' and YEAR(end_date) = '$year' ORDER BY id ASC";
        ...
        $preAmount = "SELECT SUM(credit) AS credit, SUM(debit) AS debit FROM customer_details WHERE status = 1 AND customer_id='$c_id' and YEAR(date) < '$year'";
        ...
        $details = "SELECT * FROM customer_details WHERE status=1 AND customer_id = '$c_id' and YEAR(date) = '$year' ORDER by date ASC";
